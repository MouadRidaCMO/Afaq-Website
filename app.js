/* ============================================
   AFAQ — language switcher + mobile menu
   ============================================

   French lives in the HTML as the default, so the page reads correctly
   with JavaScript disabled and search engines index it as-is. Only the
   Arabic strings live here, keyed by the element's data-i18n attribute.
   To add or edit Arabic copy, edit AR below — nothing else.
   ============================================ */
...
const AR = {
    ...
    'services.6.body': 'دعم في التداريب وأعمال الطلبة والانتقال إلى وظيفتك الأولى بعد التخرج.',
    ...
    'why.2.body': 'نعطي الأولوية لما يناسبك أكاديمياً ومهنياً، لا لمجرد التسجيل. نجاحك هو نجاحنا.',
...
/* Cairo is only fetched the first time someone actually switches to Arabic,
   so French visitors never pay for it. */
...
function setLang(lang) {
    ...
    document.querySelectorAll('[data-i18n]').forEach(el => {
        ...
        el.textContent = isAr && ar ? ar : el.dataset.fr;
    });

    const root = document.documentElement;
    root.lang = isAr ? 'ar' : 'fr';
...
function initLang() {
    ...
    // ?lang=ar in the URL wins, so a link can be shared in either language
    ...
    if (lang === 'ar' || lang === 'fr') setLang(lang);
}
